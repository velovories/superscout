#!/usr/bin/env python3
"""HackenProof program scorer.

Scrapes the public program and audit listings and ranks each program with a
simple expected-value heuristic. The main signal is dup_pays: standard bounties
pay only the first reporter, while audit contests share a pool so duplicates
still earn a share.

Usage:
  python3 programs_scout.py                 # scan, print, write json
  python3 programs_scout.py --no-others     # HackenProof only
  python3 programs_scout.py --json-only     # refresh json without the table
...
# Scoring profile. Tune weights and domain scores here.
PROFILE = {
    "our_max_reputation": 140,           # reputation held on the platform
    "domain_base": {                     # base fit score per domain (0-100)
        "smart_contract": 82, "blockchain": 78, "protocol": 80,
        "api": 48, "web": 30, "mobile": 22,
    },
    # keywords inferred from title/company/scope text
    "strong_tech": [
    ...
    "weights": {                         # blend weights (sum approx 1.0)
...
def fetch(url, key=None, cache_only=False, ttl=CACHE_TTL, validate=None, retries=0):
    """Fetch a URL with an on-disk cache.

    When validate is given, a fresh response is only accepted and cached if it
    passes the check, so an empty SPA shell never overwrites a good copy. On a
    failed or invalid fetch, retry up to `retries` times, then fall back to the
    cached copy.
    """
...
class Nuxt:
    """Resolve a Nuxt payload where every value is an index into a flat array.
    Small ints are references; containers may be tagged ['Reactive', idx]."""
...
# non-program paths the listing regex can spuriously match
_NOT_PROGRAMS = {"weekly", "new", "all", "popular", "trending", "page"}

def _page_has_program(h):
    """True if the page server-rendered a real program payload, not an empty
    SPA shell. Used as the fetch validator so a shell response is retried or
    falls back to cache instead of dropping the program."""
    if not h:
        return False
    nx = extract_nuxt(h)
    if not nx:
...
def hp_list_slugs(cache_only=False, ttl=3 * 3600):
    """Enumerate program slugs across the paginated listing until saturation.
    ttl controls listing-cache freshness; a short ttl forces a fresh re-fetch."""
    seen, empty_pages = [], 0
...
def hp_audit_list_slugs(cache_only=False, ttl=3 * 3600):
    ...
    listing. These are shared-pool contests where duplicates still pay, and they
    are not part of the /programs bounty listing. Detail pages live at
    /audit-programs/<slug> and parse with the same schema as bounties."""
    seen, empty_pages = [], 0
    for page in range(1, 13):
...
def classify_dup_pays(rec):
    ...
    Note: deposit_available is not a pool signal, it only means the program is
    funded, which is also true for many first-reporter bounties."""
    ...
    # explicit first-reporter language means a standard bounty
    ...
    # flagged audit contest without first-reporter wording is pooled
    if rec.get("is_audit"):
...
def score(rec):
    ...
    if "smart_contract" in doms:
        reasons.append("smart-contract scope")
    if "web" in doms and "smart_contract" not in doms:
        reasons.append("web-only domain")
    if weak and not strong:
        reasons.append("ecosystem outside core stack (Move/Cairo/etc.)")

    # payout_econ
    ceil = rec.get("max_bounty") or 0
    cap_score = _clamp(20 + 28 * math.log10(max(ceil, 10) / 10))   # $10 -> 20, $10k -> ~104 then clamp
    ...
    if ratio is not None and subs >= 8:
        # realized payout per submission, a reality check on the cap
        if ratio >= 250:
    ...
    # duplicate economics combined with saturation drives originality risk
    if dup_pays is True:
        ...
        if subs >= 40 or sat >= 8000:
            econ -= 18; reasons.append("first-reporter and picked-over (high duplicate risk)")
        else:
    ...
    if rec.get("reputation_required") and minrep:
        if minrep > PROFILE["our_max_reputation"]:
            acc -= 45; reasons.append(f"rep gate {minrep} above held (~{PROFILE['our_max_reputation']})")
    ...
    # a program you cannot submit to right now has no live EV
...
def _immunefi(cache_only=False):
    """Immunefi bounties listing (Next.js). Best-effort, degrades silently."""
    try:
...
def run(include_others=True, cache_only=False):
    ...
    for s in slugs:
        ...
        if rec:
            hp.append(score(rec))
            seen_slugs.add(s)
    # public audit/DualDefense contests, separate listing from /programs
    ...
    def table(title, rows):
        ...
        for r in rows:
            ...
            print(f"{sc['ev']:>4}  {_verdict_cell(r['verdict']):<17} {_dup_cell(r.get('dup_pays')):<15} "
                  ...
                  f"{rep:>4}  {'' if r.get('is_live', True) else C['y']+'[paused] '+C['x']}"
